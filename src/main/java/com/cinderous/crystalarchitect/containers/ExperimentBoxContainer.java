...
public class ExperimentBoxContainer extends Container {

    public final ExperimentBoxTileEntity tileEntity;

    private final IWorldPosCallable canInteractWithCallable;

    public ExperimentBoxContainer(final int windowId, final PlayerInventory playerInventory,
                                  final ExperimentBoxTileEntity tileEntity) {
        super(RegistryHandler.EXPERIMENT_BOX_CONTAINER.get(), windowId);
...
//        }
    ...
    private static ExperimentBoxTileEntity getTileEntity(final PlayerInventory playerInventory,
                                                         final PacketBuffer data) {
        Objects.requireNonNull(playerInventory, "playerInventory cannot be null");
        Objects.requireNonNull(data, "data cannot be null");
        final TileEntity tileAtPos = playerInventory.player.world.getTileEntity(data.readBlockPos());
        if (tileAtPos instanceof ExperimentBoxTileEntity) {
            return (ExperimentBoxTileEntity) tileAtPos;
        }
        throw new IllegalStateException("Tile entity is not correct! " + tileAtPos);
    }

    public ExperimentBoxContainer(final int windowId, final PlayerInventory playerInventory, final PacketBuffer data) {
